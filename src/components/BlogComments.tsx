...
export default function BlogComments({ blogId, initialComments = [] }: Props) {
  ...
  useEffect(() => {
    ...
    comments.forEach(c => {
      console.log(`CommentID: ${c.id}, userId: ${c.userId}, userName: ${c.user?.name}`);
    });
  ...
  const isCommentOwner = (comment: Comment) => {
    return (
      currentUserId != null &&
      (comment.user?.id == currentUserId || comment.userId == currentUserId)
    );
  };
          ...
          >
            ...
            >
              <input
                ...
                className="bg-transparent text-white placeholder-white/40 py-1 w-full focus:outline-none border-b border-transparent hover:border-white/20 transition-all duration-300 ease-in-out"
              />
              <textarea
                ref={textareaRef}
                ...
                rows={1}
                className="bg-transparent text-white placeholder-white/40 w-full resize-none overflow-hidden focus:outline-none border-b border-transparent hover:border-white/20 transition-all duration-300 ease-in-out"
              />
              <button

...
export default function LiquidEther({
  ...
  resolution = 0.5,
  isBounce = false,
  colors = ['#d946ef', '#8b5cf6', '#6366f1'], // Default to Neon
  style = {},
  className = '',
  autoDemo = true,
  autoSpeed = 0.5,
  ...
  useEffect(() => {
    ...
    class MouseClass {
      ...
      listenerTarget: Window | null = null; docTarget: Document | null = null;
    ...
    class AutoDriver {
        ...
        activationTime = 0; margin = 0.2; _tmpDir = new THREE.Vector2();
    ...
    class ShaderPass {
        ...
        material: THREE.RawShaderMaterial | null = null; geometry: THREE.PlaneGeometry | null = null; plane: THREE.Mesh | null = null;
    ...
    class WebGLManager {
        props: any; lastUserInteraction = 0; autoDriver: AutoDriver | null = null; 
        running = false; output: any;
        ...
        initOutput() {
            ...
            const fbos = {
                vel_0: createFBO(), vel_1: createFBO(),
                div: createFBO(),
                pressure_0: createFBO(), pressure_1: createFBO(),
                vel_viscous0: createFBO(), vel_viscous1: createFBO()
            };

            const advection = new ShaderPass({
                material: { vertexShader: face_vert, fragmentShader: advection_frag, uniforms: { boundarySpace: { value: cellScale }, px: { value: cellScale }, fboSize: { value: fboSize }, velocity: { value: fbos.vel_0.texture }, dt: { value: dt }, isBFECC: { value: BFECC } } },
                output: fbos.vel_1
            }); advection.init();
            ...
            const boundaryM = new THREE.RawShaderMaterial({ vertexShader: line_vert, fragmentShader: advection_frag, uniforms: advection.uniforms });
            const boundaryLine = new THREE.LineSegments(boundaryG, boundaryM);
            advection.scene?.add(boundaryLine);

            const externalForce = new ShaderPass({ output: fbos.vel_1 }); externalForce.init();
            const mouseG = new THREE.PlaneGeometry(1, 1);
            const mouseM = new THREE.RawShaderMaterial({
                vertexShader: mouse_vert, fragmentShader: externalForce_frag,
                blending: THREE.AdditiveBlending, depthWrite: false,
                uniforms: { px: { value: cellScale }, force: { value: new THREE.Vector2(0,0) }, center: { value: new THREE.Vector2(0,0) }, scale: { value: new THREE.Vector2(cursorSize, cursorSize) } }
            });
            const mouseMesh = new THREE.Mesh(mouseG, mouseM);
            externalForce.scene?.add(mouseMesh);

            const divergence = new ShaderPass({
                material: { vertexShader: face_vert, fragmentShader: divergence_frag, uniforms: { boundarySpace: { value: boundarySpace }, velocity: { value: fbos.vel_0.texture }, px: { value: cellScale }, dt: { value: dt } } },
                output: fbos.div
            }); divergence.init();

            const poisson = new ShaderPass({
                material: { vertexShader: face_vert, fragmentShader: poisson_frag, uniforms: { boundarySpace: { value: boundarySpace }, pressure: { value: fbos.pressure_0.texture }, divergence: { value: fbos.div.texture }, px: { value: cellScale } } },
                output: fbos.pressure_1
            }); poisson.init();

            const pressure = new ShaderPass({
                material: { vertexShader: face_vert, fragmentShader: pressure_frag, uniforms: { boundarySpace: { value: boundarySpace }, pressure: { value: fbos.pressure_0.texture }, velocity: { value: fbos.vel_0.texture }, px: { value: cellScale }, dt: { value: dt } } },
                output: fbos.vel_0
            }); pressure.init();
            ...
            this.output = {
                resize: () => {
                    ...
                    fboSize.set(w, h); cellScale.set(1.0/w, 1.0/h);
                    Object.values(fbos).forEach(f => f.setSize(w, h));
                    advection.uniforms.fboSize.value = fboSize; advection.uniforms.px.value = cellScale; advection.uniforms.boundarySpace.value = cellScale;
                    mouseMesh.material.uniforms.px.value = cellScale;
                    divergence.uniforms.px.value = cellScale; poisson.uniforms.px.value = cellScale; pressure.uniforms.px.value = cellScale;
                },
                update: () => {
                    if (isBounce) boundarySpace.set(0,0); else boundarySpace.copy(cellScale);
                    advection.uniforms.dt.value = dt; advection.uniforms.isBFECC.value = BFECC;
                    boundaryLine.visible = isBounce;
                    advection.update();
                    ...
                    mouseMesh.material.uniforms.force.value.set(forceX, forceY);
                    mouseMesh.material.uniforms.center.value.set(centerX, centerY);
                    mouseMesh.material.uniforms.scale.value.set(cursorSize, cursorSize);
                    externalForce.update();

                    divergence.uniforms.velocity.value = externalForce.props.output.texture; 
                    divergence.uniforms.boundarySpace.value = boundarySpace;
                    divergence.update();

                    // FIXED: Initialized p_out to prevent undefined error
                    let p_in, p_out = fbos.pressure_1;
                    for(let i=0; i<iterationsPoisson; i++) {
                        if(i%2===0){ p_in = fbos.pressure_0; p_out = fbos.pressure_1; }
                        else { p_in = fbos.pressure_1; p_out = fbos.pressure_0; }
                        poisson.uniforms.pressure.value = p_in.texture;
                        poisson.props.output = p_out;
                        poisson.update();
                    }
                    
                    pressure.uniforms.velocity.value = fbos.vel_1.texture;
                    pressure.uniforms.pressure.value = p_out.texture;
                    pressure.uniforms.boundarySpace.value = boundarySpace;
                    pressure.update();

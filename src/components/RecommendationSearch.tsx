...
export default function RecommendationSearch() {
  ...
  const fetchResults = async () => {
    setLoading(true);
    const res = await fetch(`/api/search/${type === 'MOVIE' ? 'tmdb' : 'spotify'}?q=${encodeURIComponent(query)}`);
  ...
  return (
    <div className="max-w-md mx-auto p-4 space-y-4">
      ...
      {/* Search Input */}
      {type === 'SONG' ? (
        <div className="rounded-2xl border border-white/10 bg-white/[0.03] p-6 md:p-8 space-y-3 overflow-hidden relative">
          <div className="flex items-center gap-2 text-xs uppercase tracking-[0.28em] text-white/45">
            <span className="h-2 w-2 rounded-full bg-white/80" />
            Songs
          </div>
          <h3 className="text-3xl md:text-4xl leading-none text-white font-semibold tracking-tight">Coming Soon</h3>
          <p className="max-w-xl text-sm md:text-base leading-7 text-white/65">
            This music shelf is paused for now. The same theme stays in place, but song search and submission are hidden until the full experience is ready.
          </p>
        </div>
      ) : (
        <>
